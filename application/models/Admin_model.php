...

{
	...
	public function get_user_from_not_in_group($id) 
	{

		return $this->db->select("ID, username")
		->where('ID not in', '(select `userid` from `user_group_users` where `groupid` = '.$id.')', false)
		->group_by('users.ID')
		->get("users");
	}
	...
	public function company_list(){
		$query=$this->db->select('SendCompany')->where('SendCompany !=', '')->group_by('SendCompany')->get('dawnwingapi');
		return $query->result();
	}
